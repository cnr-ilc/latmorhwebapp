...
//        }
//        if (typeOfAnalysis == 4) {
//            //System.err.println("CCCCCCCCCCCCCCCCCC");
//            /*complete analysis*/
//            try {
//                root.getChildren().add(createCompleteTree(obj));
//            } catch (Exception e) {
...
//                        __LEMMAS__ = __LEMMAS__ + totLemmas + ") ";
                            ...
                            temp = new DefaultTreeNode(__LEMMA__, nodelemma);
                            try {
            ...
            if (node == null) {
                ...
                for (JsonValue value : analyses) {
                    ...
                    for (JsonValue lemma : lemmas) {
                        ...
                        __LEMTYPE__ = objLemmas.getString(labelLemType);
                        if (typeOfAnalysis == 3) {
                            __LEMMA__ = "From Analysis: " + (i + 1) + " " + __LEMMA__ + " ";
                        }

                        JsonArray codes = objLemmas.getJsonArray(labelMorfCodes);
                        //PoS = codes.getJsonObject(0).getString(__CF__);//codes.getJsonObject(0).getString(labelPos);
                        try {
                            PoS = codes.getJsonObject(0).getString(labelPos) + " (" + codes.getJsonObject(0).getString(__CF__) + ")";
                        } catch (Exception e) {
                            PoS = codes.getJsonObject(0).getString(labelPos);
                        }
...
//                            pubw.flush();
    ...
    private String createTheResponse(List<JsonObject> jsons) {
        String labelNof = "Number_of_Forms";
        String labelNodf = "Number_of_Distinct_Forms";
        String labelNol = "Number_of_Lemmas";
        String labelNodl = "Number_of_Distinct_Lemmas";
        String labelNofnf = "Number_of_Forms_Not_Found";
        String labelNolf = "Number_of_Lemmatized_Forms";
        String labelStat = "stat";
        String labelResults = "results";
        String ret = "";
        ...
        statBuilder.add(labelNof, numForm);
        statBuilder.add(labelNodf, numDistinctForm);
        statBuilder.add(labelNol, numLemmas);
        statBuilder.add(labelNodl, getNumGlobalLemmas());
        statBuilder.add(labelNofnf, getNumFormNotFound());
        statBuilder.add(labelNolf, getNumLemmatizedForm());
        responseBuilder.add(labelStat, statBuilder);
        for (JsonObject obj : jsons) {
            resultsArrayBuilder.add(obj);
        }
        responseBuilder.add(labelResults, resultsArrayBuilder);

        ret = responseBuilder.build().toString();
        return ret;

    }

    private void returnJson(String ret) {
